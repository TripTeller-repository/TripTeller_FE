...
import useImgUpload from "/src/hooks/useImgUpload";
    ...
    try {
      ...
      if (url) {
        if (uploadType === "profile") {
          // 프로필 이미지 등록 API 호출
          const response = await fetch(`${URL}/user/profile-image`, {
            method: "POST",
            headers: {
          ...
          onClose();
        } else if (uploadType === "log") {
          // 로그 이미지 등록, 여기서도 비슷한 로직을 적용할 수 있습니다.
          onClose();
        }
      } else {
