...
const convertToKST = (utcDateString) => {
  const date = new Date(utcDateString); // 서버로부터 받은 UTC 형식의 날짜를 Date 객체로 변환
  const kstOffset = 9 * 60; // UTC+9, 분 단위로 변환
  const localTime = new Date(date.getTime() + kstOffset * 60 * 1000); // KST로 변환
  return localTime;
};
...
const ScheduleContent = () => {
  ...
  const scrollContainerBy = (distance) => {
    if (centerContainerRef.current) {
      centerContainerRef.current.scrollBy({
        left: distance,
        behavior: "smooth",
      });
    }
  ...
  useEffect(() => {
    const onMounted = async () => {
      const response = await request(
        `/my-trip/${feedId}/travel-plan/${travelPlan_Id}`,
      );
  ...
  const handleCreateSchedule = (newSchedule, planId) => {
    setDailyPlans((prevDailyPlans) => {
      ...
      const dayIndex = updatedPlans.findIndex(
        (d) => d._id === (newSchedule.dayId || planId),
      );

      if (dayIndex >= 0) {
        // 해당 일자가 존재하면 새 일정을 해당 일자의 dailySchedules 배열에 추가합니다.
        updatedPlans[dayIndex].dailySchedules = [
          ...updatedPlans[dayIndex].dailySchedules,
          newSchedule,
        ];
      }
  ...
  const handleDeleteSchedule = async (scheduleId, planId) => {
    ...
    try {
      ...
      const response = await fetch(
        `${URL}/daily-plan/${planId}/daily-schedule/${scheduleId}`,
        {
          method: "DELETE",
          headers: {
      ...
      setDailyPlans((prevDailyPlans) => {
        return prevDailyPlans.map((plan) => {
          ...
          if (plan._id === planId && Array.isArray(plan.dailySchedules)) {
            return {
              ...plan,
              dailySchedules: plan.dailySchedules.filter(
